...
"""
    ...
    def _extract_keywords(self, text: str) -> List[str]:
        ...
        theme_keywords = {
            ...
            "若手離職防止": ["若手", "離職", "定着", "若手離職"],
            "中堅離職防止": ["中堅", "中堅層", "離職", "中堅離職", "中堅層離職"],
    ...
    def _search_knowledge(self, query: str) -> List[Dict]:
        ...
        for file_info in files:
            ...
            matched_theme = None
            for theme, keywords in theme_keywords.items():
                # クエリとファイルの両方でテーマキーワードをチェック
                query_matches = sum(1 for keyword in keywords if keyword in query.lower())
                file_matches = sum(1 for keyword in keywords if keyword in file_title.lower() or keyword in file_content.lower())
                
                # より多くのキーワードがマッチするほど高スコア
                if query_matches > 0 and file_matches > 0:
                    score += 300 + (query_matches * 50) + (file_matches * 30)
                    matched_theme = theme
                    break
